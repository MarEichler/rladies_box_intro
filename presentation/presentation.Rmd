...
<br><br><br>
<center>Let's Learn how to use <code>{box}</code><center>
...
<div id = "contentbox2", style = "width: 100%;">
  ...
  <div id="column2" style="float:right; width:49%;">
    ...
    <ul>
      ...
      <li>Would not be able to use <code>mutate()</code></li>
      <li>Could use <code>dplyr::mutate()</code> to use </li>
    </ul>
  </div>
</div>
...
with CRAN packages use double colon `::` to call a function from a specific package, with modules use `$` 
